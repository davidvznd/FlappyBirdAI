public class Obstacle {
  ...
  Obstacle(PVector pos, PVector oSpeed, int lastObs)
  {
    ...
    speed = oSpeed;
    yUp = randomNumber - 145;
    yDown = randomNumber + 145;
  ...
  void checkCollision(Bird bird)
  {
    ...
    if (bird.position.x + bird.birdWidth >= position.x - offset + 8 && bird.position.x <= position.x + offset -8 && bird.position.y + bird.birdHeight >= yDown + 6 && bird.position.y + bird.birdHeight<= height - 112)
    {
      bird.isAlive = false;
      bird.reference = this;
    }
    if (bird.position.x + bird.birdWidth >= position.x - offset + 6 && bird.position.x <= position.x + offset -6 && bird.position.y >= 0 && bird.position.y <= yUp - 8)
    {
      bird.isAlive = false;
      bird.reference = this;
    }
